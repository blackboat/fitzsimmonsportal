...
class WC_XR_Invoice {
	...
	public function set_org_default_due_date( $due_date, $wc_xr_invoice ) {
		...
		$transient_key = 'wc_xero_org_due_date_' . md5( serialize( $this->settings ) );
		...
		if ( ! empty ( $xml_response->Organisations->Organisation->PaymentTerms->Sales->Type )  ) {
			...
			switch( $type ) {
				case 'OFFOLLOWINGMONTH':
					$due_date = date( 'Y-m', strtotime( 'today +1 month' ) ) . '-' . $day;
					break;
				case 'OFCURRENTMONTH':
					$due_date = date( 'Y-m' ) . '-' . $day;
					break;
				case 'DAYSAFTERBILLMONTH':
					$due_date = date( 'Y-m-d', strtotime( date('Y-m-t') . " +$day days" ) );
					break;
				case 'DAYSAFTERBILLDATE':
					$due_date = date( 'Y-m-d', strtotime( $this->due_date . " +$day days" ) );
					break;
			}
		}

		set_transient( $transient_key, $due_date, 31 * DAY_IN_SECONDS );
		return $due_date;
	}
